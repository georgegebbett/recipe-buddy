...
describe('RecipeScraper', () => {
  ...
  const belliniRecipe = new Recipe();
  belliniRecipe.name = 'Bellini';
  belliniRecipe.url = 'https://www.bbcgoodfood.com/recipes/bellini';
  belliniRecipe.imageUrl =
    'https://images.immediate.co.uk/production/volatile/sites/30/2020/08/bellini-b049342.jpg';
  belliniRecipe.ingredients = [
    '500ml peach purée or peach nectar',
    '1 bottle prosecco',
  ];
  belliniRecipe.steps = [
  ...
  it('should get a NodeList from a given URL', async function () {
    jest
      ...
      .mockResolvedValueOnce(belliniNodeList());
    const foundNodeList = await scraper.getNodeListOfMetadataNodesFromUrl(
      belliniUrl,
    );
    expect(foundNodeList).toEqual(belliniNodeList());
  });

  it('should extract the Recipe object from a NodeList', function () {
    expect(scraper.getSchemaRecipeFromNodeList(belliniNodeList())).toEqual(
  ...
  it('should be able to return a recipe object from a url', async function () {
    ...
    expect(await scraper.hydrateRecipe(belliniUrl)).toEqual(belliniRecipe);
  });
