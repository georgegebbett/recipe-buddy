...
const productMapper = (product: Product): ComboboxOption => ({
  key: product.id,
  label: product.name,
});

export const IngredientTable: FC<IngredientTableProps> = ({
  ingredients,
  products,
}) => {
  return (
        ...
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {ingredients.map((ingredient) => (
              <IngredientTableRow ingredient={ingredient} products={products} />
            ))}
          </tbody>
        </table>
      </div>
...
const IngredientTableRow: FC<IngredientTableRowProps> = ({
  ingredient,
  products,
}) => {
  ...
  return (
    <tr key={ingredient}>
      ...
      <td className="px-3 py-4 text-sm text-gray-500">{ingredient}</td>
